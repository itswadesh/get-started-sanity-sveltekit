...
export const fetchSettings = async () => {
  try {
    const data = (await get('es/settings')).data?.settings
    settings.set(data)
...
export const fetchStore = async () => {
  try {
    ...
    store.set(data)
    await get('store-one', { id: storeOne.id })
    const data1 = (await get('megamenu', { store: storeOne.id })).data?.megamenu //get('es/categories/megamenu', { megamenu: true })
    megamenu.set(data1)
  } catch (e) {
    store.set({})
  }
