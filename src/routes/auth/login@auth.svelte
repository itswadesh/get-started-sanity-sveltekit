...
<script>
  ...
  async function handleSignIn() {
    try {
      ...
      $session.me = me
      let r = ref || '/my'
      // await invalidate('/auth/login')
      if (browser) goto(r, { replaceState: true })
    } catch (e) {
    } finally {
      loading = false
    }
...
{#if !me}
  <div class="grid items-center justify-center">
    <div
      class="frosted flex w-full max-w-sm flex-col rounded-2xl border bg-cover bg-center bg-no-repeat px-4 py-10 shadow-2xl sm:px-8"
    >
      <!-- Store logo/name  -->
      ...
      <a href="/" class="mx-auto mb-5 max-w-max">
        ...
        {:else if store?.websiteName}
          <h1
            class="from-secondary-500 to-primary-500 bg-gradient-to-br bg-clip-text text-4xl font-extrabold text-transparent"
          >
            {store?.websiteName}
          </h1>
        {/if}
      ...
      <h6 class="mb-3 text-center text-gray-600">
      ...
      <form class="mb-8 flex flex-col" on:submit|preventDefault={handleSignIn}>
        ...
        <GradiantButton class="mx-auto w-full max-w-sm" type="submit" {loading}>
          LOGIN
        </GradiantButton>
      </form>
      ...
      <!-- mb-5 -->
      <div
        class="mx-auto flex w-full max-w-sm flex-wrap items-center justify-center gap-4 leading-4"
      >
        <a
          href={`/auth/otp-login?ref=${
